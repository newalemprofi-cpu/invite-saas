...
export function SectionDecoration({
  decorationId, label, accent, color, className = "mb-4",
}: {
  ...
  switch (decorationId) {
    ...
    case "classic-hairline":
      return (
        <div className={`flex flex-col items-center gap-2.5 ${className}`}>
          <p className="invite-kicker" style={{ color: textColor }}>{label}</p>
    ...
    case "none":
    default:
      return <p className={`invite-kicker ${className}`} style={{ color: textColor }}>{label}</p>;
  }
}
