...
export interface CategoryCardData {
  ...
  count: number;
  /** Admin-editable override (Site CMS → "Мереке категориялары") for this locale; already resolved to the canonical label if the admin left it empty. */
  title: string;
  /** Admin-editable override for this locale; empty string means "render no description". */
  description: string;
}
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 sm:px-6 py-14 sm:py-20">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
        {cards.map(({ category, coverUrl, count, title, description }, i) => (
          <Link
            key={category.id}
          ...
          >
            ...
            <div className="absolute inset-x-0 bottom-0 p-3 sm:p-4 flex items-end justify-between gap-2">
              <div className="min-w-0">
                ...
                {description && (
                  <p className="text-white/75 text-xs mt-0.5 line-clamp-2 leading-snug">{description}</p>
                )}
                {count > 0 && (
