  ...
  return (
    <section className="relative overflow-hidden py-16 sm:py-24 text-center px-4" style={{ background: "var(--charcoal)" }}>
      ...
      <div className="relative max-w-xl mx-auto flex flex-col items-center gap-6">
        ...
        {/* Two ways to order: the website flow stays visually primary — the
            WhatsApp path is a clearly secondary, smaller, outlined action
            directly beneath it, never replacing the SaaS flow. */}
        <div className="flex flex-col items-center gap-3">
          <Link href={`/templates?lang=${lang}`} className="btn-gold text-base px-8 py-3.5">
            {t.cta} →
          </Link>
          <p className="text-xs" style={{ color: "rgba(250,248,243,0.4)" }}>{t.orLabel}</p>
          <a
            href={waHref}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-colors hover:bg-white/5"
            style={{ border: "1px solid rgba(250,248,243,0.25)", color: "rgba(250,248,243,0.85)" }}
          >
